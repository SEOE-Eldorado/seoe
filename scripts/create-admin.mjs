...
async function main() {
  ...
  try {
    ...
    console.log("UID:", uid);
    console.log("");
    console.log("Podés loguearte en: admin.2.25.132.129.nip.io/login");
    process.exit(0);
  } catch (err) {
    console.error("❌ Error:", err.message);
    if (err.code === 'auth/email-already-in-use') {
      console.error("El email ya existe. Intentá con otro o eliminá el usuario desde Firebase Console.");
    }
    process.exit(1);
  }
}
